...
class Recorder(object):
    ...
    def record(self, prefix, step=-1, loss_stats=None, image_stats=None):
        ...
        if self.use_wandb:
            log_dict = {pattern.format(k): v.median if isinstance(v, SmoothedValue) else v for k, v in loss_stats.items()}
            wandb.log({**log_dict, 'global_step': step})
...
def write_summary(result_dir, summary_dict, hparam_dict, use_wandb=False):
    # save to tensorboard
    writer = SummaryWriter(log_dir=result_dir)
    writer.add_hparams(metric_dict=summary_dict, hparam_dict=hparam_dict)
